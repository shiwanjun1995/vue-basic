...
```js
watch：精准盯梢，就像个专业的侦探，只盯着你指定的目标
允许我们更细粒度地响应状态的变化，它可以监听一个响应式对象的变化，当对象的某个属性发生改变时，会触发回调函数。
watchEffect：自动雷达，就像个自动扫描仪，自动发现所有相关的东西
则会自动监听所有在回调函数中使用的响应式对象，当这些对象发生改变时，会触发回调函数。
```
